[![NPM](https://nodei.co/npm/buckscashd-ha.png?downloads=true&stars=true)](https://nodei.co/npm/buckscashd-ha/)

[![Build Status](https://travis-ci.org/brandonlehmann/buckscashd-ha.png?branch=master)](https://travis-ci.org/brandonlehmann/buckscashd-ha) [![Build Status](https://ci.appveyor.com/api/projects/status/github/brandonlehmann/buckscashd-ha?branch=master&svg=true)](https://ci.appveyor.com/project/brandonlehmann/buckscashd-ha/branch/master)

# bucksdaemon High-Availability Daemon Wrapper

This project is designed to wrap the bucksdaemon daemon on a *nix system and monitor it for hangups, locks, fork, or other events that cause the daemon to stop responding to requests in an accurate manner.
...
5. [Documentation](#documentation)
   1. [Methods](#methods)
   2. [Events](#events)
   3. [bucksdaemon RPC API Interface](#buckscashd-rpc-api-interface)
...
* [bucksdaemon](https://github.com/buckscash/bucks/releases) v1.2.2 or higher

## Easy Start

You *must* copy ```bucksdaemon``` into the ```buckscashd-ha``` folder for the easy start process to occur.

```bash
git clone https://github.com/buckscash/bucks/buckscashd-ha.git
cd buckscashd-ha
cp <bucksdaemon> .
sudo npm install & node service.js
```

**It is highly recommended that you use [checkpoints](https://github.com/buckscash/buckscash/wiki/Using-checkpoints) when starting fresh or you'll need to wait a while for the sync to occur.**
...
pm2 start service.js --name buckscashd
pm2 save
```
...
Practically all bucksdaemon command line arguments are exposed in the constructor method. Simply include them in your list of options to get activate or use them. Default values are defined below.

```javascript
var daemon = new bucksdaemon({
  // These are our bucksdaemon-ha options
  ...
  path: '~/bucks/bucksdaemon', // Where can I find bucksdaemon?
  ...
  feeAddress: Ze3GLbHkwq6iDt65iX6Nejb6LbYuvBSJ97Hjz3qoFKtwKwnBHVkTFyESL2Wt37kZHSFc9fvLoxCbE4soTJ7BjzCC3A5bhbQYS, // allows to specify the fee address for the node
...
This event is emitted when the daemon has lost synchronization with the bucks network
...
This event is emitted when the daemon is synchronized with the bucks network and is passing all the checks we have for it. It returns the equivalent of a */getinfo* call to the RPC server with a few minor additions.
...
This event is emitted when the daemon starts. The callback contains the command line arguments supplied to bucksdaemon.
...
This event is emitted when the daemon has synchronized with the bucks network.
