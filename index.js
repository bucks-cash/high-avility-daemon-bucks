...
const bucksdaemonRPC = require('./lib/bucksdaemon-rpc.js').bucksdaemon;
...
const bucksdaemon = function (opts) {
  opts = opts || {};
  if (!(this instanceof bucksdaemon)) return new bucksdaemon(opts);
  ...
  */
  ...
  // Begin bucksdaemon options
  this.path = opts.path || path.resolve(__dirname, '/root/bucks/bucksdaemonaemon/bucksdaemon'); // + ((os.platform() === 'win32') ? '.exe' : ''));
  this.dataDir = opts.dataDir || path.resolve(os.homedir(), '/root/.bucks'); // /root/.bucks/DB
  this.logFile = opts.logFile || path.resolve(__dirname, '/root/.bucks/bucksdaemon.log');
  this.logLevel = opts.logLevel || 2;
  this.testnet = opts.testnet || false;
  this.enableCors = "*" || false; //false;
  ...
  this.rpcBindPort = opts.rpcBindPort || 21698;
  this.p2pBindIp = opts.p2pBindIp || false;
  this.p2pBindPort = opts.p2pBindPort || 21688;
...
inherits(bucksdaemon, EventEmitter)

bucksdaemon.prototype.start = function () {
  ...
  this.emit('info', 'Attempting to start buckscashd-ha...');
...
bucksdaemon.prototype.stop = function () {
...
bucksdaemon.prototype.write = function (data) {
  this._write(util.format('%s\r', data))
}

bucksdaemon.prototype._checkChildStdio = function (data) {
...
bucksdaemon.prototype._triggerDown = function () {
...
bucksdaemon.prototype._triggerUp = function () {
...
bucksdaemon.prototype._checkServices = function () {
  if (!this.synced) {
    this.synced = true
...
bucksdaemon.prototype._checkRpc = function () {
...
bucksdaemon.prototype._checkDaemon = function () {
  return new Promise((resolve, reject) => {
    this.help = false
    this.write('help')
    setTimeout(() => {
...
bucksdaemon.prototype._write = function (data) {
  this.child.write(data)
}

bucksdaemon.prototype._buildargs = function () {
  var args = ''
...
bucksdaemon.prototype._setupAPI = function () {
  this.api = new bucksdaemonRPC({
    host: this.rpcBindIp,
...
bucksdaemon.prototype._setupWebSocket = function () {
...
bucksdaemon.prototype._registerWebSocketClientEvents = function (socket) {
  var that = this
